...
(function () {
  ...
  function snapshotCard(item) {
    return {
      ...
      images: snapshotImages(item?.images, item?.image),
      alt: text(item?.alt),
  ...
  function snapshotImages(images, fallbackImage) {
    const fallback = text(fallbackImage);
    ...
    return {
      original: text(images.original || fallback),
      source: text(images.source),
      thumb: text(images.thumb),
      medium: text(images.medium || fallback),
      large: text(images.large || fallback)
    };
  }
