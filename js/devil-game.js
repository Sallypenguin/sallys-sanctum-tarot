...
(() => {
  ...
  const DEVIL_GAME_BUILD = "v71-cache-bust";
...
ふむ。実に人間らしい。`;
  ...
  function getDevilNonBustDefeatLine() {
    ...
    if (isTargetScore(gameState.playerTotal)) {
      return `君は66、私は${gameState.devilTotal}。見事だ。門の真上に立って、なお声を荒げないとは。今夜は君に道を譲ろう。`;
    }

    if (winningGap > 0 && winningGap <= 2) {
      return `君は${gameState.playerTotal}、私は${gameState.devilTotal}。わずかな差だな。だがそのわずかを君が握った。今夜は、私の指の方が扉に挟まれたようだ。`;
    }
  ...
  function getDevilVictoryLine({ playerBust, devilBust, perfectBustLoss } = {}) {
    ...
    if (perfectBustLoss) {
      ...
      line = `さきほど66を越えた君を、契約は${gameState.playerTotal}まで戻した。だが、戻れたことと勝てることは別の話だ。君は${gameState.playerTotal}、私は${gameState.devilTotal}。私の勝ちだ。`;
