...
(() => {
  ...
  const DEVIL_GAME_BUILD = "v139_target-stand-confirm-lines";
...
破滅しか残らない道を、館の主は選ばなかった。`;
  ...
  const STAND_CONFIRMATION_LINE_LAYERS = {
    relaxed: {
      target: ({ total }) => `66で留まるのか。合計は${total}。見事だ。そこまで来れば、余計な一枚は飾りにもならない。では、その完全な数で、私に届くか試してみたまえ。`,
    ...
    standard: {
      target: ({ total }) => `66で留まるのだな。合計は${total}。最も正しい停止位置だ。では、その数が私に届くか、見届けるとしよう。`,
    ...
    serious: {
      target: ({ total }) => `66で来るのだな。合計は${total}。よろしい。その数を、君の答えとして受け取るとしよう。`,
...
対象カード：『${cardName}』`;
  ...
  function getStandConfirmationContext(state = gameState) {
    ...
    const target = Number(state?.target) || TARGET_SCORE;
    const risk = getNextPlayerBustRisk(state);
    const band = isTargetScore(total) ? "target" : getBustRiskBand(risk.rate);
    return {
      total,
      target,
      risk,
