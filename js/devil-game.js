...
ふむ。実に人間らしい。`;
  ...
  function finishGame() {
    ...
    if (gameState.winner === "player") {
      ...
      if (tiedWithoutBust) {
        const tieLine = gameState.playerTotal === gameState.target
          ? TARGET_TIE_DEVIL_LINE
          : `同点だ。君は${gameState.playerTotal}、私は${gameState.devilTotal}。悪魔の館では、引き分けは私のもの。`;
        addLog(`${tieLine} 勝負は終わった。だが、${contractedCardName}に残った印は、勝敗より長く残るだろう。`);
      } else if (playerBust) {
        addLog(perfectBustLoss ? PERFECT_BUST_DEVIL_LINE : `君は66を越えた。私の勝ちだ。勝負は終わった。だが、${contractedCardName}に残った印は、勝敗より長く残るだろう。`);
      } else {
      ...
      if (gameState.playerTotal === gameState.target) {
        addLog(TARGET_TIE_DEVIL_LINE);
      } else {
        addLog(`同点だ。君は${gameState.playerTotal}、私は${gameState.devilTotal}。悪魔の館では、引き分けは私のもの。そういう規則なのだよ。`);
      }
    } else if (playerBust) {
      addLog(perfectBustLoss ? PERFECT_BUST_DEVIL_LINE : "君は66を越えた。私の勝ちだ。欲張りは嫌いではないが、勘定は別だ。");
    } else {
...
けれど、${getContractedCardThisGameName()}だけは以前の姿では戻らなかった。`;
  ...
  async function runDebugScenario(id) {
    const scenarios = {
      ...
      perfect66BustImmediate: async () => {
        setDebugState({ name: "66ジャストから一枚引いてバースト、契約済みのため即敗北", player: [21, 20, 19, 6], devil: [0, 2], deck: [1], contractUsed: true });
        await debugPlayerDraw();
      },
      targetTie: async () => {
        setDebugState({ name: "66同士で同点、悪魔勝利", player: [21, 20, 19, 6], devil: [18, 17, 16, 15], deck: [], hiddenRevealed: true });
        await debugStandThroughResult();
      },
  ...
  function createDebugPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="devil-debug-panel__grid" aria-label="デバッグシナリオ">
