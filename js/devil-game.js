...
(() => {
  ...
  const DEVIL_GAME_BUILD = "v74-devil-ai-contract";
...
ふむ。実に人間らしい。`;
  ...
  function getDevilTurnOpeningLine(profile = DEVIL_AI_PROFILES.standard) {
    ...
    if (isTargetScore(playerTotal)) {
      return chooseLine([
        "君は66。ならば私は、66に届くしかない。選択肢が細いほど、遊戯は美しい。",
        "66に辿り着いたか。よかろう。ここからは、私にも余計な道は残されていない。"
      ]);
    }

    if (playerTotal >= 63) {
      return chooseLine([
        `君は${playerTotal}で止まった。いい場所だ。ここから先は、私も雑には札へ触れられない。`,
    ...
    if (profile.id === "mercy") {
      if (playerTotal <= 35) {
        return chooseLine([
          `そこで足を止めるのか。よかろう。館は広い、追いつく時間はいくらでもある。`,
          `合計は${playerTotal}。勝つだけなら静かに済ませればいい。だが、それでは少し退屈だな。`
        ]);
      }
      return chooseLine([
        "勝つだけなら、静かに済ませればいい。だが今夜は、少し音が欲しい。",
...
対象カード：『${cardName}』`;
  ...
  async function proceedToDevilTurn() {
    ...
    while (shouldDevilDraw(gameState, profile)) {
      ...
      if (isBust(gameState.devilTotal, profile.bustLimit)) {
        addLog(`悪魔は66を越えた。合計は${gameState.devilTotal}。`);
        render();
        await delay(120);
        break;
      }
  ...
  function getDevilBustDefeatLine() {
    ...
    if (gameState.playerTotal >= 63) {
      return `君は${gameState.playerTotal}で踏みとどまり、私は66を越えた。あと一歩踏みとどまるべきだったのは、どうやら私の方だったな。`;
    }
  ...
  function getDevilVictoryLine({ playerBust, devilBust, perfectBustLoss } = {}) {
    ...
    if (perfectBustLoss) {
      ...
      line = isTargetScore(gameState.playerTotal)
