...
ふむ。実に人間らしい。`;
  ...
  async function playerDraw() {
    ...
    if (isBust(gameState.playerTotal, gameState.target)) {
      if (isContractedCard(card)) {
        addLog(`${drawMessage} その札は、すでに私の印を受けている。同じ札で二度救われるつもりかね？`);
        render();
        playFlipSound();
        await delay(getDealDelay() + MODAL_DELAY_MS);
        gameState.winner = "devil";
        gameState.resultType = "loss-marked";
        gameState.resultModalDismissed = false;
        setPhase("result");
        recordGameStats();
        setInteractionLocked(false);
      ...
      if (!gameState.contractUsed) {
        addLog(`${drawMessage} 66を越えたな。契約するかね？　最後の一枚を消してやろう。代わりに、その札へ私の印を置く。なあに、それだけのことだ。`);
        render();
        playFlipSound();
      ...
      addLog(`${drawMessage} 再び66を越えた。二度目の幕引きだ。`);
      render();
      playFlipSound();
      await delay(getDealDelay() + MODAL_DELAY_MS);
      finishGame();
  ...
  function rejectContract() {
    ...
    addLog("拒むのかね？　よかろう。負けを自分のものにする者を、私は嫌いではない。");
    gameState.winner = "devil";
    gameState.resultType = "loss-reject";
    gameState.resultModalDismissed = false;
    setPhase("result");
    recordGameStats();
    render();
  ...
  function finishGame() {
    ...
    gameState.resultType = `${gameState.winner === "player" ? "win" : "loss"}-${contractKey}`;
    gameState.resultModalDismissed = false;
    setPhase("result");
    recordGameStats();
    ...
    if (gameState.winner === "player") {
      if (gameState.contractUsed) {
        if (devilBust) {
          addLog(`私は66を越えた。君の勝ちだ。よかろう、${contractedCardName}の印まで含めて、今夜は君のものということにしておこう。`);
        } else {
          addLog(`君は${gameState.playerTotal}、私は${gameState.devilTotal}。君の勝ちだ。よかろう、${contractedCardName}の印まで含めて、今夜は君のものということにしておこう。`);
        }
      } else if (devilBust) {
        addLog(`私は66を越えた。君の勝ちだ。ふむ、今夜の見世物は私だったようだな。`);
      } else {
        addLog(`君は${gameState.playerTotal}、私は${gameState.devilTotal}。君の勝ちだ。ふむ、今夜は君の読みが勝ったようだ。`);
      }
    } else if (gameState.contractUsed) {
      if (tiedWithoutBust) {
        addLog(`同点だ。君は${gameState.playerTotal}、私は${gameState.devilTotal}。悪魔の館では、引き分けは私のもの。勝負は終わった。だが、${contractedCardName}に残った印は、勝敗より長く残るだろう。`);
      } else if (playerBust) {
        addLog(`君は66を越えた。私の勝ちだ。勝負は終わった。だが、${contractedCardName}に残った印は、勝敗より長く残るだろう。`);
      } else {
        addLog(`君は${gameState.playerTotal}、私は${gameState.devilTotal}。私の勝ちだ。勝負は終わった。だが、${contractedCardName}に残った印は、勝敗より長く残るだろう。`);
      }
    } else if (tiedWithoutBust) {
      addLog(`同点だ。君は${gameState.playerTotal}、私は${gameState.devilTotal}。悪魔の館では、引き分けは私のもの。そういう規則なのだよ。`);
    } else if (playerBust) {
      addLog(`君は66を越えた。私の勝ちだ。欲張りは嫌いではないが、勘定は別だ。`);
    } else {
      addLog(`君は${gameState.playerTotal}、私は${gameState.devilTotal}。私の勝ちだ。よかろう、今夜は私の館らしい幕引きだ。`);
    }
  }
...
勝負は終わった。だが、${getContractedCardThisGameName()}に残った印はまだ終わっていない。`;
  ...
  function renderResult() {
    ...
    if (isResult) {
      const winnerText = playerWon ? "あなたの勝ち" : "私の勝ち";
  ...
  function render() {
    ...
    const isPreparingCards = gameState.phase === "loading" || gameState.phase === "dealing";
    const hasGameStarted = gameState.phase !== "intro";
    if (dom.startGameButton) {
      dom.startGameButton.hidden = false;
      dom.startGameButton.disabled = isPreparingCards || locked;
      dom.startGameButton.textContent = isPreparingCards
        ? "札を準備中…"
        : hasGameStarted
          ? "最初から"
          : "ゲームを始める";
