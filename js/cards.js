...
(function () {
  ...
  function imageFor(details, variant = "medium", cardId = "") {
    const source = details?.image || details?.images?.original || details?.images?.medium || "";
    if (window.SallyDevilContracts?.getDisplayImageSet) {
      const images = window.SallyDevilContracts.getDisplayImageSet(source, cardId);
      return images?.[variant] || images?.medium || source || "";
    }
    const images = details?.images || (window.SallyImageAssets?.getImageSet ? window.SallyImageAssets.getImageSet(source) : null);
  ...
  function contractSuffix(cardId) {
    return isCardContracted(cardId) ? "。契約の刻印あり" : "";
  }

  function contractAlt(alt, cardId) {
    return isCardContracted(cardId) ? `${alt}。悪魔の契約の刻印があります。` : alt;
  }

  function contractBadge(cardId) {
    return isCardContracted(cardId) ? '<span class="contract-seal-badge">契約</span>' : "";
  }
      ...
      <div class="arcana-thumb-list">
        ${cards.map((card) => {
          ...
          return `
            <button
              type="button"
              class="arcana-thumb-button ${contracted ? "is-contracted" : ""}"
              data-card-id="${escapeHtml(card.id)}"
              aria-label="${escapeHtml(details.label)}へ移動${contractSuffix(card.id)}"
              title="${escapeHtml(details.label)}${contractSuffix(card.id)}"
            >
              <img src="${escapeHtml(imageFor(details, "thumb", card.id))}" alt="" aria-hidden="true" width="240" height="360" loading="lazy" decoding="async">
  ...
  function renderCard(card, details) {
    ...
    return `
      <article class="cards-index-card ${contracted ? "is-contracted" : ""}" id="${escapeHtml(getCardElementId(card.id))}" data-card-id="${escapeHtml(card.id)}" tabindex="-1">
        <button
          type="button"
          class="cards-card-image image-zoom-button js-open-image-dialog ${contracted ? "is-contracted" : ""}"
