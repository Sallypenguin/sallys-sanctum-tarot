...
(function () {
  ...
  function imageFor(item, variant = "medium") {
    const images = item?.images || (window.SallyImageAssets?.getImageSet ? window.SallyImageAssets.getImageSet(item?.image) : null);
    return images?.[variant] || item?.image || "";
  }
      ...
      <article class="slot ${reversedClass}" data-card-id="${escapeHtml(item.id)}" data-detail-target="${escapeHtml(item.position.id)}" style="--slot-index:${index}">
        ...
        <button type="button" class="card-stage js-reveal-card" aria-label="${escapeHtml(item.label)}をめくる">
          <span class="card" aria-hidden="true">
            ...
            <span class="face front"><img src="${escapeHtml(imageFor(item, "medium"))}" alt="${escapeHtml(item.alt)}" width="640" height="960" decoding="async" fetchpriority="high"></span>
          </span>
        </button>
    ...
    return `
      <article class="detail-panel card-detail ${active ? "is-active" : ""}" data-detail-panel="${escapeHtml(panelKey)}" role="tabpanel" ${getPanelStateAttributes(panelKey, active)}>
        <header class="card-detail-header">
          ...
          >
            <img src="${escapeHtml(imageFor(item, "medium"))}" alt="${escapeHtml(item.alt)}" width="640" height="960" loading="lazy" decoding="async">
    ...
    return `
      <figure class="pair-mini-card ${item.orientation === "reversed" ? "reversed" : ""}">
        <button
          ...
          aria-label="${escapeHtml(item.label)}の画像を拡大表示"
        >
          <img src="${escapeHtml(imageFor(item, "thumb"))}" alt="${escapeHtml(item.alt)}" width="240" height="360" loading="lazy" decoding="async">
        </button>
        <figcaption>
