...
(function () {
  ...
  function renderEmptyState(message = "まだカードは引かれていません。スプレッドと相談テーマを選んで、カードを引いてください。") {
      ...
      <section class="detail-reader" aria-label="リーディング本文">
        <nav class="detail-tabs" role="tablist" aria-label="表示する解説を選択">
          ${tabItems.map((tab, index) => `
            <button
              type="button"
              class="detail-tab js-detail-tab ${index === 0 ? "is-active" : ""}"
        ...
        <div class="detail-carousel-shell" aria-label="解説パネルのカルーセル">
          <div class="detail-panels detail-carousel js-detail-carousel" tabindex="0" aria-live="polite">
            ${renderOverviewPanel(reading)}
            ${reading.cards.map((item) => renderCardDetailPanel(item)).join("")}
            ${reading.pairs.length ? renderSynergyPanel(reading.pairs) : ""}
          </div>
        </div>
      </section>
    `;
  ...
  function renderOverviewPanel(reading) {
    return `
      <article class="detail-panel reading-overview is-active" data-detail-panel="overview" role="tabpanel">
        <h2>リーディング概要</h2>
        <div class="overview-grid">
          <div><span>スプレッド</span>${reading.spreadType === "three" ? "三枚引き" : "一枚引き"}</div>
  ...
  function renderCardDetailPanel(item) {
    return `
      <article class="detail-panel card-detail" data-detail-panel="${escapeHtml(item.position.id)}" role="tabpanel">
  ...
  function renderSynergyPanel(pairs) {
    return `
      <article class="detail-panel pair-section" data-detail-panel="synergy" role="tabpanel">
        <h2>カード同士のシナジー</h2>
