...
(function () {
  ...
  async function draw(spreadType) {
    if (isDrawing) return;
    isDrawing = true;
    setDrawControlsDisabled(true);
    spreadEl.innerHTML = '<div class="empty-state" role="status" aria-live="polite">カード画像を読み込んでいます……</div>';
    ...
    try {
      ...
      await prepareReadingImages(reading);
      ...
      startRevealSequence(reading);
    } catch (error) {
      console.error(error);
      spreadEl.innerHTML = window.TarotRender.renderEmptyState("カードの読み込み中に問題が起きました。ページを再読み込みして、もう一度試してください。");
    } finally {
  ...
  function startRevealSequence(reading) {
    ...
    slots.forEach((slot, index) => {
      ...
      if (button) {
        button.addEventListener("click", (event) => {
          ...
          revealSlot(slot);
          selectDetailPanel(slot.dataset.detailTarget, { scrollToTabs: true });
        });
      }
  ...
  function getDisplayImageForReadingItem(item, variant = "medium") {
    const source = item?.card?.media?.image
      || item?.card?.image
      || item?.image
      || item?.images?.original
      || item?.images?.medium
      || "";
    const cardId = item?.id || item?.card?.id || "";

    if (window.SallyDevilContracts?.getDisplayImageSrc) {
      return window.SallyDevilContracts.getDisplayImageSrc(source, cardId, variant);
    }

    return item?.images?.[variant] || item?.image || source;
  }

  function getDisplayImageForCard(card, variant = "medium") {
    const source = card?.media?.image || card?.image || "";
    if (window.SallyDevilContracts?.getDisplayImageSrc) {
      return window.SallyDevilContracts.getDisplayImageSrc(source, card?.id, variant);
    }
    return window.TarotEngine.getCardDetails(card).images?.[variant] || source;
  }

  function prepareReadingImages(reading) {
    const assetHelper = window.SallyImageAssets;
    if (!assetHelper || typeof assetHelper.preloadImages !== "function") return Promise.resolve([]);

    const frontImages = (reading?.cards || []).map((item) => getDisplayImageForReadingItem(item, "medium")).filter(Boolean);
    const backImage = assetHelper.getCardBackImage ? assetHelper.getCardBackImage("medium") : window.TAROT_CARD_BACK;
    return assetHelper.preloadImages([backImage, ...frontImages], 9000);
  }

  function setDrawControlsDisabled(disabled) {
    ["#draw-one", "#draw-three", "#draw-triad"].forEach((selector) => {
  ...
  function scheduleDeckCacheWarmup() {
    const assetHelper = window.SallyImageAssets;
    if (!assetHelper || typeof assetHelper.warmImageCache !== "function") return;

    const startWarmup = () => {
      const urls = cards
        .map((card) => getDisplayImageForCard(card, "medium"))
        .filter(Boolean);
      assetHelper.warmImageCache(urls, { delayMs: 700 });
    };
  ...
  function bindDetailReader() {
    ...
    tabs.forEach((tab) => {
      ...
      tab.addEventListener("keydown", (event) => handleDetailTabKeydown(event, tabs));
    });

    const activeTab = tabs.find((tab) => tab.getAttribute("aria-selected") === "true") || tabs[0];
    if (activeTab) setActiveDetailKey(activeTab.dataset.detailKey);
  ...
  function handleDetailTabKeydown(event, tabs) {
    ...
    if (!nextTab) return;
    selectDetailPanel(nextTab.dataset.detailKey);
    nextTab.focus({ preventScroll: true });
  }

  function selectDetailPanel(key, options = {}) {
    if (!key) return;
  ...
  function scrollToDetailTabs() {
    const target = spreadEl.querySelector(".detail-tabs") || spreadEl.querySelector(".detail-reader");
    if (!target) return;
  ...
  function setActiveDetailKey(key) {
    ...
    panels.forEach((panel) => {
      ...
      setPanelFocusability(panel, active);
    });
  }

  function setPanelFocusability(panel, active) {
    if (!panel) return;
  ...
  function updateDetailCarouselHeight() {
    ...
    // Use the panel border-box height instead of scrollHeight: scrollHeight excludes
    // borders and can round down fractional pixels, which clips the bottom border.
    const panelHeight = activePanel.getBoundingClientRect().height;
    carousel.style.height = `${Math.ceil(panelHeight)}px`;
  }
